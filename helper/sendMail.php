...
function sendMailOrder($mail, $receiver, $content) {
    try {
        ...
        $mail->Port = 587; //465
        $mail->Username = 'luanlan0166@gmail.com';
        $mail->Password = 'wxzu xdof fhro zyxs';                          
        
        // Recipients
        $mail->setFrom('luanlan0166@gmail.com', 'HCMUT_ADMIN');
        $mail->addAddress($receiver['email'], $receiver['name']);     
        $mail->addReplyTo('luanlan0166@gmail.com', 'HCMUT_ADMIN');

        // Content
        $mail->isHTML(true);                                  
        $mail->Subject = 'HCMUT_ADMIN xác nhận đơn hàng #'.$receiver['id'];
        $mail->Body = ' <html>
                            <body>
                                <h3>Xin chào '.$receiver['name'].'</h3>
                                <p>Cảm ơn quý khách đã đặt hàng tại <a href="#">HCMUT_ADMIN</a>.</p>
                                <p>Đơn hàng quý khách sẽ sớm được gửi đi sau khi nhân viên của chúng tôi hoàn tất các thủ tục.</p>
                                <div>'.$content.'</div>
                                <p>Mọi thắc mắc xin vui lòng liên hệ qua gmail: luanlan0166@gmail.com</p>
                                <p>Xin kính chúc sức khỏe và may mắn!</p>
                                <p><b style="color: blue">HCMUT_ADMIN</b></p>
                            </body>
                        </html>';
...
function verifyEmail($mail, $receiver, $verifyCode) {
    try {
        ...
        // Content
        $mail->isHTML(true);                                  
        $mail->Subject = 'HCMUT_ADMIN xác thực tài khoản tạo mới';
        $mail->Body = ' <html>
                            <body>
                                <h3>Xin chào '.$receiver['name'].'</h3>
...
function resetPassword($mail, $receiver) {
    try {
        ...
        // Content
        $mail->isHTML(true);                                  
        $mail->Subject = 'HCMUT_ADMIN xác thực thông tin tài khoản';
        $mail->Body = ' <html>
                            <body>
                                <h3>Xin chào '.$receiver['name'].'</h3>
...
function sendLink2ChangePWD($mail, $receiver, $content) {
    try {
        ...
        // Content
        $mail->isHTML(true);                                  
        $mail->Subject = 'HCMUT_ADMIN gửi link thay đổi mật khẩu';
        $mail->Body = ' <html>
                            <body>
                                <h3>Xin chào '.$receiver['name'].'</h3>
